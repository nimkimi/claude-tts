...
- **Snapshot build (`_snapshot_state`)** holds `self._lock` to build the full JSON-shaped dict via each object's `to_state()` (reading every `HistoryEntry` field under the lock, so a concurrent `heard`-flip can't tear the read) and captures `mono_now`/`wall_now`. **The lock is released BEFORE the disk write — no disk I/O ever happens under the lock (the load-bearing rule).** The in-lock dict build is a bounded pass (sub-millisecond-to-low-ms) over ≤ `history_cap` × sessions small entries, at a debounced cadence, never during an utterance; the guards suite confirms no arbitration regression. A two-phase extract-under-lock / shape-off-lock split is a deferred optimization, unnecessary at this scale. No nested lock is taken, so no deadlock (verified in review).
...
3. **A SIGTERM handler makes that `finally` actually run on a real install.** `sonari install` tears the daemon down with `launchctl unload`, which sends **SIGTERM**; Python's *default* SIGTERM handler terminates the process **without running `finally`**, so the graceful flush above would be dead code on the exact restart path SP6 exists for. `run()` therefore installs a SIGTERM handler (`_install_signal_handlers`) that clears `_running` + sets `_wake` (signal-safe: Event ops only), dropping the loop into the `finally` → stop/join/flush. Best-effort (`signal.signal` is main-thread-only; a non-main caller degrades to periodic-only). This closes the last-<`debounce`s-of-output loss on install; a genuine **crash** (SIGKILL, uncatchable) still relies on the periodic writer, which already covers it.
