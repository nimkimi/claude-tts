...
- **`SessionStart`** — *owner-approved deviation from the "no decision ⇒ async" rule, 2026-08-11.* The rule is about **blocking**; this is about **ordering**. `SessionStart` establishes session identity, and it is the one event whose handling is order-sensitive rather than eventually-consistent. It fires once per session, so it contributes nothing to the responsiveness problem this task solves (which comes from high-frequency events: MessageDisplay, PreToolUse, Stop) — keeping it synchronous costs nothing and removes a class of ordering risk rather than betting on harness semantics.

  > **CORRECTED 2026-08-11 (T18 review).** The original rationale here — and in commit `5142d3a`'s message — claimed the risk was an async `SessionStart` *losing a race to `UserPromptSubmit`*. **That mechanism is wrong.** `hooks_entry.handle_event("SessionStart", …)` emits **two** messages from a single hook invocation, `SET_FOREGROUND` then `SESSION_START`, sent as two fire-and-forget `client.send()` calls. The first already `_record()`s the session, so `is_new` (`lifecycle.py:64`) is already False by the time the real `SESSION_START` is handled. The decision to keep this sync stands — it is free and harmless — but it is not protecting against what the original text said. See the follow-up below; the announce it was meant to protect **is already broken**, independently of async.
