...
def isolate_paths(root, monkeypatch=None) -> None:
    ...
    # GENERATE its 4.8 MB silent WAV over the developer's real
    # ~/.sonari/keepalive.wav — and a test that writes a short stub there leaves
    # the live daemon a truncated file to hand afplay. Landed with the keep-alive
    # work directly in conftest.py's old inline list, which this module replaced;
    # it has to be re-stated HERE or the move silently drops it.
    _setattr(paths, "KEEPALIVE_WAV_PATH", sonari_dir / "keepalive.wav")
