...
# ---------------------------------------------------------------------------
# ⌃⌘Tab — on_cycle_session cycles FROM speaker()
# ---------------------------------------------------------------------------

def test_cycle_session_from_speaker_not_foreground_under_divergence():
    """⌃⌘Tab under divergence: cycles from B (speaker) → C, not from A → B."""
    daemon, queue, speaker, sessions, config = make_daemon(foreground="A")
    sessions.register("B", cwd="/x/B")
    ...
    # Without repoint: fg=A(idx 0) → target=B; speaker=="B", not "C" → RED.
    # With repoint:    fg=B(idx 1) → target=C; speaker=="C" → GREEN.
