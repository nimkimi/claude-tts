...
def test_permission_request_handler_fn_name():
    import sonari.daemon  # noqa: F401
    import sonari.daemon.registry as reg
    fn = reg.HANDLERS.get(_MsgType.PERMISSION_REQUEST)
    assert fn is not None
    assert fn.__name__ == "on_permission_request"


def test_answer_permission_handler_fn_name():
    import sonari.daemon  # noqa: F401
    import sonari.daemon.registry as reg
    fn = reg.HANDLERS.get(_MsgType.ANSWER_PERMISSION)
    assert fn is not None
    assert fn.__name__ == "on_answer_permission"
