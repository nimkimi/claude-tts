...
def on_set_foreground(ctx, msg):
    ...
    if t == MsgType.SESSION_START:
        ...
        if st is not None and st.announce_resume:
            ...
            # SET_FOREGROUND call) -- an ARM-TIME proof only. Task 10 / R21
            # closed the arm-to-deliver gap this proof left open:
            # on_stop_session's stopping branch and on_stop_all now
            # invalidate the flag the instant the stream is stopped again, so
            # by the time this leg runs the flag is already False on any
            # since-stopped stream -- this delivery can no longer land on
            # one. And it is the right direction for a control cue anyway:
            # it exists because he pressed a key, so it is delivered
